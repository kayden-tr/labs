...
function _m_make_snapshot {
  HOST=$1
  options=$2
  indices=$(curl -s http://$HOST:$PORT/_cat/indices | awk '{print $3}' | grep -vE '^\.\w+' | tr '\n' ',' | sed 's/,$//')
  curl -m 30 -XPUT http://$HOST:$PORT/_snapshot/daily/${DATE}?$options -H 'Content-Type: application/json' -d '{
    "indices": "'$indices'",
